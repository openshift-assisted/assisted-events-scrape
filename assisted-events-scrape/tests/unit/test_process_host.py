...
class TestHostProcess:
    ...
    def test_reshape_host(self):
        ...
        host = get_host(inventory=None)
        reshape_host(host)
        assert "inventory" not in host
        assert "host_inventory" in host
        assert host["host_inventory"] is None

        host = get_host(inventory="")
        reshape_host(host)
        assert "inventory" not in host
        assert "host_inventory" in host
        assert host["host_inventory"] == ""
